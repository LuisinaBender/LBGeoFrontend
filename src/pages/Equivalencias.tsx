...
const Equivalencias: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingEquivalencia) {
        await equivalenciasApi.update(editingEquivalencia.id_equivalencia, formData);
      } else {
        await equivalenciasApi.create({ ...formData, eliminado: false });
      }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('¿Está seguro de eliminar esta equivalencia?')) {
      try {
        await equivalenciasApi.delete(id);
        fetchEquivalencias();
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        <h3 className="text-lg font-semibold text-slate-900 mb-4">Buscador Inteligente de Equivalencias</h3>
        <div className="flex gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="pl-10 pr-4 py-3 w-full border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
                ...
                <div key={equiv.id_equivalencia} className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <div className="font-mono text-slate-900 bg-white px-2 py-1 rounded border">
                        {equiv.codigo_OEM_original}
                      </div>
                    </div>
                    <div>
                      <span className="text-sm text-slate-600">Equivalente:</span>
                      <div className="font-mono text-slate-900 bg-white px-2 py-1 rounded border">
                        {equiv.codigo_OEM_equivalente}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {searchTerm && searchResults.length === 0 && (
          <div className="mt-4 p-4 bg-slate-50 rounded-lg text-center">
            <p className="text-slate-600">No se encontraron equivalencias para el código "{searchTerm}"</p>
          </div>
        )}
      </div>
              ...
              {filteredEquivalencias.map((equiv) => (
                <tr key={equiv.id_equivalencia} className="hover:bg-slate-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                    {equiv.id_equivalencia}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="font-mono text-sm text-slate-900 bg-slate-100 px-2 py-1 rounded">
                      {equiv.codigo_OEM_original}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="font-mono text-sm text-slate-900 bg-blue-100 px-2 py-1 rounded">
                      {equiv.codigo_OEM_equivalente}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-2">
                      ...
                      <Button
                        variant="danger"
                        size="sm"
                        onClick={() => handleDelete(equiv.id_equivalencia)}
                      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              required
            />
            <p className="text-xs text-slate-500 mt-1">Código original del fabricante</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              required
            />
            <p className="text-xs text-slate-500 mt-1">Código equivalente de otro fabricante</p>
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <div className="flex items-center space-x-2">
              <span className="font-mono text-sm bg-white px-2 py-1 rounded border">
                {formData.codigo_OEM_original || 'Original'}
              </span>
              <RotateCcw className="w-4 h-4 text-blue-600" />
              <span className="font-mono text-sm bg-white px-2 py-1 rounded border">
                {formData.codigo_OEM_equivalente || 'Equivalente'}
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button type="submit">
              {editingEquivalencia ? 'Actualizar' : 'Crear'}
            </Button>
          </div>
        </form>
      </Modal>
